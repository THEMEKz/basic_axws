// Jenkinsfile Template – Enable CI/CD pipeline into axws Project.
// Version: 3.0.0
// Copyright © 2022 AtlasX Developer
//
// This template can only be used for projects that are axws v3.0.0,
// which contains web application (Angular) and backend (.NET 5),
...
pipeline {
    ...
    environment {
        ...
        IIS_API_SERVER_URL = "http://appserv1.cdg.co.th/iis-api-server"
    ...
    stages {
        ...
        stage('Build') {
            steps {
                dir('ServerApp') {
                    dotnetClean sdk: 'dotnet 5'
                    dotnetPublish sdk: 'dotnet 5', configuration: 'Release', outputDirectory: '../out', runtime: 'win-x64'
                }

                dir('ClientApp') {
                    nodejs(nodeJSInstallationName: 'node 14', configId: 'atlasx-npmrc') {
                        sh 'npm ci'
